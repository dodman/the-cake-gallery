      ...
      {items.length === 0 ? (
        ...
        <div className="mt-8 grid gap-6 lg:grid-cols-[1fr_320px]">
          <div className="grid gap-4">
            {items.map((item) => (
              <div key={item._id} className="grid grid-cols-[96px_1fr] gap-4 rounded-lg bg-white p-4">
                ...
                <div>
                  <div className="flex justify-between gap-3">
                    ...
                    <button onClick={() => removeItem(item._id)} aria-label="Remove item"><Trash2 size={18} /></button>
                  </div>
                  <p className="mt-1 text-sm text-cocoa/60">{money(item.price)}</p>
                  <div className="mt-4 flex items-center gap-2">
                    <Button variant="ghost" className="px-3 py-2" onClick={() => updateQuantity(item._id, item.quantity - 1)}><Minus size={16} /></Button>
                    <span className="w-8 text-center font-semibold">{item.quantity}</span>
                    <Button variant="ghost" className="px-3 py-2" onClick={() => updateQuantity(item._id, item.quantity + 1)}><Plus size={16} /></Button>
                  </div>
                </div>
              </div>
            ))}
