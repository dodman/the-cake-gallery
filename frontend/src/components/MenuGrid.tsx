  ...
  return (
    <section className="mx-auto max-w-7xl px-4 py-12">
      ...
      <div className="mt-8 flex gap-2 overflow-x-auto pb-2">
        ...
        {categories.map((item) => (
          <button key={item.id} onClick={() => setCategory(item.slug)} className={`whitespace-nowrap rounded-md px-4 py-2 text-sm font-semibold ${category === item.slug ? "bg-cocoa text-white" : "bg-white"}`}>{item.name}</button>
        ))}
      </div>
